...
<nav class="flex" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                <i class="fas fa-chevron-right text-gray-400 text-xs mx-2"></i>
                <span class="text-sm text-pc-orange font-medium">Editar Despliegue</span>
            </div>
        </li>
    </ol>
</nav>
    ...
    <div class="card-pc">
        ...
        <div class="bg-pc-blue px-6 py-4 flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-bold text-white uppercase tracking-tight">Editar Despliegue Operativo</h2>
                    <p class="text-blue-100 text-xs">Misión: {{ $deployment->place }}</p>
                </div>
            </div>
        ...
        <form id="deployment-form" action="{{ route('deployments.update', $deployment) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="place" id="place" required placeholder="Ej: Jardín Botánico, Ciudad Bolívar"
                                   class="input-pc @error('place') border-red-500 @enderror" value="{{ old('place', $deployment->place) }}">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="latitude" id="latitude" x-model="latitude" placeholder="8.1234" class="input-pc" value="{{ old('latitude', $deployment->latitude) }}">
                            </div>
                            <div>
                                <label for="longitude" class="label-pc">Longitud (opcional)</label>
                                <input type="text" name="longitude" id="longitude" x-model="longitude" placeholder="-63.1234" class="input-pc" value="{{ old('longitude', $deployment->longitude) }}">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="division" id="division" placeholder="Ej: Operaciones, Rescate, Gestión de Riesgo"
                                   class="input-pc" value="{{ old('division', $deployment->division) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="reason" id="reason" rows="3" required placeholder="Describa el objetivo y motivo de la misión..."
                                      class="input-pc @error('reason') border-red-500 @enderror">{{ old('reason', $deployment->reason) }}</textarea>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="datetime-local" name="start_datetime" id="start_datetime" required
                                       class="input-pc @error('start_datetime') border-red-500 @enderror" 
                                       value="{{ old('start_datetime', $deployment->start_datetime ? $deployment->start_datetime->format('Y-m-d\TH:i') : '') }}">
                            </div>
                            <div>
                                ...
                                <input type="datetime-local" name="end_datetime" id="end_datetime" 
                                       ...
                                       class="input-pc" value="{{ old('end_datetime', $deployment->end_datetime ? $deployment->end_datetime->format('Y-m-d\TH:i') : '') }}">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="supervisor_id" id="supervisor_id" required class="input-pc">
                                ...
                                @foreach($employees as $emp)
                                    <option value="{{ $emp->id }}" {{ old('supervisor_id', $deployment->supervisor_id) == $emp->id ? 'selected' : '' }}>
            ...
            <div class="p-8 border-t border-gray-100 bg-gray-50/20">
                ...
                <textarea name="notes" id="notes" rows="2" placeholder="Información técnica, equipo especial requerido, contactos locales, etc."
                          class="input-pc">{{ old('notes', $deployment->notes) }}</textarea>
            </div>
        </form>
    </div>
</div>
...
<script>
    function deploymentForm() {
        return {
            isIndefinite: {{ $deployment->is_indefinite ? 'true' : 'false' }},
            searchTerm: '',
            isLoading: false,
            suggestions: [],
            selectedParticipants: @json($deployment->participants->map(fn($p) => [
                'id' => $p->id,
                'full_name' => $p->full_name,
                'id_number' => $p->id_number,
                'role' => $p->pivot->role ?? '',
                'division' => $p->pivot->division ?? '',
                'is_leader' => (bool)($p->pivot->is_leader ?? false)
            ])),
            participantError: '',
            latitude: '{{ old('latitude', $deployment->latitude) }}',
            longitude: '{{ old('longitude', $deployment->longitude) }}',
            map: null,
            marker: null,
            ...
            initMap() {
                setTimeout(() => {
                    const initialLat = this.latitude || 8.1283;
                    const initialLng = this.longitude || -63.5497;
                    
                    this.map = L.map('map-picker').setView([initialLat, initialLng], 13);
