    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-pc-blue">Editar Rotación</h2>
            </div>

            <form action="{{ route('guard-rotations.update', $guardRotation) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $guardRotation->name) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-pc-orange focus:ring focus:ring-pc-orange focus:ring-opacity-50">{{ old('description', $guardRotation->description) }}</textarea>
                </div>

                <div class="mb-4">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $guardRotation->is_active) ? 'checked' : '' }}
                ...
                <div class="border-t border-gray-200 pt-6 mt-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach(['a' => 'A', 'b' => 'B', 'c' => 'C', 'd' => 'D'] as $key => $letter)
                            <div x-data="{
                                open: false,
                                search: '{{ old("employee_{$key}_id", $guardRotation->{"employee_{$key}_id"}) ? ($employees->firstWhere('id', old("employee_{$key}_id", $guardRotation->{"employee_{$key}_id"}))?->id_number . ' - ' . $employees->firstWhere('id', old("employee_{$key}_id", $guardRotation->{"employee_{$key}_id"}))?->full_name) : '' }}',
                                selectedId: '{{ old("employee_{$key}_id", $guardRotation->{"employee_{$key}_id"}) }}',
                                results: [],
                                loading: false,
                                fetchResults() {
